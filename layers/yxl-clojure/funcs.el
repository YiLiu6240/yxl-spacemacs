...
(defun clojure/cider-eval-current-form-sp (&optional arg)
  "A clojure version for `spacemacs/eval-current-form-sp'."
  (interactive "p")
  (require 'smartparens)
...
(defun clojure/cider-send-current-form-sp-to-repl (&optional arg)
  "A clojure version for `spacemacs/eval-current-form-sp', send to REPL."
  (interactive "p")
  (require 'smartparens)
  (let ((evil-move-beyond-eol t))
    ;; evil-move-beyond-eol disables the evil advices around eval-last-sexp
    (save-excursion
      (let ((max 10))
        (while (and (> max 0)
          ...
          (sp-up-sexp)))
      (sp-up-sexp arg)
      (call-interactively 'spacemacs/cider-send-last-sexp-to-repl))))
